...
eles) void broadcast_message(int num, int sender_id){ for(int i=0;
i<clients.size(); i++) if(clients[i].id!=sender_id)
    ...
    cout << "\nCAST ID PARA CHAR INT:" << id << "CHAR :"<< id_char << "\n";
       ...
       //if(bytes_received<=0)
        ...
        if(strcmp(str,"#exit")==0){
                        ...
                        string message=string("-")+string(name)+string(" saiu do
chat!"); broadcast_message("#NULL",id); broadcast_message(id,id);
...
int main(){
    ...
    unsigned int len = sizeof(sockaddr_in);
        ...
        if((bind(server_socket,(struct sockaddr *)&server,sizeof(struct
sockaddr_in)))==-1){ perror("bind error: "); exit(0);
        }
...
*)&client,&len);
        ...
        if(client_socket == -1){
            ...
            printf("Conectado ao cliente\n");
                ...
                lock_guard<mutex> guard(clients_mtx);
                clients.push_back({contagem_cliente,
string("Anonymous"),client_socket,(move(t))});
        }
